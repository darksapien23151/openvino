...
int64_t DefineBufferClusters::get_buffer_finalization_offset(const LoopManagerPtr& loop_manager, const BufferExpressionPtr& buffer_expr) const {
    auto get_direct_loop = [&](const ExpressionPort& consumer) -> UnifiedLoopInfoPtr {
        ...
        if (idx == buffer_loops_ids.size() && idx == consumer_loop_ids.size())
            return nullptr;
        OPENVINO_ASSERT(idx < consumer_loop_ids.size(), "Incorrect index");
        return loop_manager->get_loop_info<UnifiedLoopInfo>(consumer_loop_ids[idx]);
    };
    ...
    for (const auto& buffer_out : buffer_outs) {
        ...
        for (const auto& consumer : consumers) {
            if (const auto& direct_loop = get_direct_loop(consumer)) {
...
bool DefineBufferClusters::unite_nested_clusters(const BufferClusters::iterator& inner_cluster_it,
    ...
    for (const auto& inner_buffer : *inner_cluster_it) {
        ...
        if (are_buffer_neighbours(up_buffer, down_buffer, common_loop_end_expr, up_idx, down_idx)) {
            ...
            const auto up_port_desc = LoopPortInfo(LoopPort(), {inner_ptr_increments[up_idx], inner_final_offsets[up_idx], inner_data_sizes[up_idx]});
            const auto down_port_desc = LoopPortInfo(LoopPort(), {inner_ptr_increments[down_idx], inner_final_offsets[down_idx], inner_data_sizes[down_idx]});
            if (SetBufferRegGroup::can_be_in_one_group(up_port_desc, down_port_desc)) {
                for (const auto& inner_buffer : *inner_cluster_it)
